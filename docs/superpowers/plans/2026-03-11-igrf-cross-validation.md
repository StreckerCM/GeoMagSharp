# IGRF Cross-Validation Tests Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Cross-validate GeoMagSharp's IGRF-12 and IGRF-14 implementations against NOAA's authoritative IGRF-14 calculator, covering 6 epochs, 3 locations, and 2 altitudes.

**Architecture:** Single test file `IgrfCrossValidationTest.cs` with parameterized `[DataRow]` tests following the WMM2025ValidationTest pattern. Phase 1 uses IGRF12.COF (4 epochs), Phase 2 uses IGRF14.COF (6 epochs). Two tolerance tiers: tight for definitive epochs, loose for non-definitive.

**Tech Stack:** C# / MSTest / .NET Framework 4.8 / GeoMagSharp library

**Spec:** `docs/superpowers/specs/2026-03-11-igrf-cross-validation-design.md`
...
|------|--------|---------------|
| `tests/GeoMagSharp.Tests/IgrfCrossValidationTest.cs` | Create | All IGRF cross-validation test methods |
| `tests/GeoMagSharp.Tests/TestData/IGRF12.COF` | Copy from `coefficient/IGRF12.COF` | Phase 1 coefficient data |
| `tests/GeoMagSharp.Tests/TestData/IGRF14.COF` | Copy from `coefficient/IGRF14.COF` | Phase 2 coefficient data |
| `docs/features/12-igrf-cross-validation/tasks.md` | Create | Ralph Loop tracking |

---

## Chunk 1: Setup and Phase 1 Main Field Tests

### Task 1: Copy IGRF Coefficient Files to TestData

**Files:**
- Copy: `coefficient/IGRF12.COF` → `tests/GeoMagSharp.Tests/TestData/IGRF12.COF`
- Copy: `coefficient/IGRF14.COF` → `tests/GeoMagSharp.Tests/TestData/IGRF14.COF`

- [ ] **Step 1: Copy coefficient files**

```bash
cp coefficient/IGRF12.COF tests/GeoMagSharp.Tests/TestData/IGRF12.COF
cp coefficient/IGRF14.COF tests/GeoMagSharp.Tests/TestData/IGRF14.COF
```

- [ ] **Step 2: Verify files copied correctly**

```bash
head -3 tests/GeoMagSharp.Tests/TestData/IGRF12.COF
head -3 tests/GeoMagSharp.Tests/TestData/IGRF14.COF
```

Expected: Each file starts with a model header line (e.g., `    IGRF   ...` or similar `.COF` format header).

- [ ] **Step 3: Commit**

```bash
git add tests/GeoMagSharp.Tests/TestData/IGRF12.COF tests/GeoMagSharp.Tests/TestData/IGRF14.COF
git commit -m "test: copy IGRF12 and IGRF14 COF files to TestData for cross-validation"
```

### Task 2: Create Test File with ClassInitialize and Phase 1 Main Field Tests

**Files:**
- Create: `tests/GeoMagSharp.Tests/IgrfCrossValidationTest.cs`
- Reference: `tests/GeoMagSharp.Tests/WMM2025ValidationTest.cs` (pattern to follow)

- [ ] **Step 1: Create the test file with ClassInitialize, tolerance constants, and Phase 1 main field DataRows**

Create `tests/GeoMagSharp.Tests/IgrfCrossValidationTest.cs` with this content:
...
Run: `dotnet test -c Release --filter "FullyQualifiedName~IgrfCrossValidation" --verbosity normal`
Expected: 120 tests execute. Definitive-epoch tests should pass at tight tolerances. Non-definitive epoch (2015.0 IGRF-12) tests may need tolerance adjustments — if they fail, investigate the magnitude of differences before loosening.

- [ ] **Step 4: Commit**

```bash
git add tests/GeoMagSharp.Tests/IgrfCrossValidationTest.cs
git commit -m "test: add IGRF cross-validation tests (Issue #12)

Phase 1: 48 tests cross-validating IGRF-12 against NOAA IGRF-14 reference
Phase 2: 72 tests validating IGRF-14 against NOAA reference values
120 total parameterized test cases across 6 epochs, 3 locations, 2 altitudes"
```

### Task 3: Create Ralph Loop tasks.md

**Files:**
- Create: `docs/features/12-igrf-cross-validation/tasks.md`
...
## Tasks
- [ ] Copy IGRF12.COF and IGRF14.COF to TestData
- [ ] Create IgrfCrossValidationTest.cs with Phase 1 + Phase 2 tests
- [ ] Verify all 120 tests pass
- [ ] Verify build succeeds on both net48 and netstandard2.0
...
```bash
git add docs/features/12-igrf-cross-validation/tasks.md
git commit -m "docs: add tasks.md for Issue #12 IGRF cross-validation"
```

### Task 4: Tolerance Adjustment (if needed)

This task only applies if tests fail in Task 2 Step 3.

- [ ] **Step 1: Analyze failures**

If any tests fail, check the actual vs expected values. For definitive epochs (2000, 2005, 2010), differences > 1.0 nT indicate a potential implementation issue — investigate before loosening tolerances. For non-definitive epoch 2015.0, differences up to 50 nT are expected.

- [ ] **Step 2: Adjust tolerances if necessary**

Only adjust the `Loose*` constants. Never loosen tight tolerances without understanding why.

- [ ] **Step 3: Re-run and commit**

```bash
dotnet test -c Release --filter "FullyQualifiedName~IgrfCrossValidation" --verbosity normal
git add tests/GeoMagSharp.Tests/IgrfCrossValidationTest.cs
git commit -m "test: adjust IGRF cross-validation tolerances based on actual results"
```
